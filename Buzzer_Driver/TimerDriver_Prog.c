...
const static u8 TimerDrive_PrescalerVal[5] = {1, 2, 3, 4, 5};
#define  ISR(vector, ...)            \
...
extern u8 Timer_u8SetTimeMsec(u8 Copy_u8Time_msec)
{
	...
	local_u8CountsNum = (local_u8fnDoneRight) * (MC_WORKING_FREQ_KHZ * Copy_u8Time_msec) / (local_u8Prescaller_Val);
...
extern u8 Timer_u8SetTimeUsec(u16 Copy_u8Time_usec)
{
	...
	local_u8CountsNum = (local_u8fnDoneRight) * (MC_WORKING_FREQ_MHZ * Copy_u8Time_usec) / (local_u8Prescaller_Val);
	*TIMER0_DATA_REG = 0;
	*TIMER0_COMPARE_REG = local_u8CountsNum;
	return local_u8fnDoneRight;
}
...
{
	*TIMER0_CONTROL_REG &= 0b11111000;
	*TIMER0_CONTROL_REG |= TimerDrive_PrescalerVal[Copy_u8PrescallerIndex];
}
